...
def main():
	...
	if restart == 1:
		...
		n_chains, n_cell, read_file_path = read_cond()
...
class InitialSetup:
	...
	def calc_conditions(self):
		if self.nw_type == "NoLJ_Harmonic":
			bond = 0.97
			c_inf = 1.0
		elif self.nw_type == "KG" or self.nw_type == "LJ_Harmonic":
			bond = 0.97
			c_inf = 1.7
			target_density = 0.85
		...
		if self.nw_type == "KG" or self.nw_type == "LJ_Harmonic" or self.nw_type == "Atr_LJ_w_Ang":
...
class ModifyTop:
	...
	def make_histgram(self, candidate_list, target_dir):
		...
		#
		hist_data_path = os.path.join(target_dir, "hist.dat")
		self.write_data(hist_data_path, hist_data)
		#
		plt_path = os.path.join(target_dir, "make_hist.plt")
		png_path = os.path.join(target_dir, "Histgram.png")
		self.make_graph(plt_path, hist_data_path, png_path, bin_width)
		return x, val
	
	# ヒストグラムのデータを書き出し 
	def write_data(self, file, list):
	...
	###########################################################
	#----- グラフを作成
	def make_graph(self, plt_file, hist_data_path, png_path, bin_width):
		self.make_script(plt_file, hist_data_path, png_path, bin_width)
	...
	# 必要なスクリプトを作成
	def make_script(self, plt_file, hist_data_path, png_path, bin_width):
		with open(plt_file, 'w') as f:
			script = self.script_content(bin_width, hist_data_path, png_path)
			f.write(script)
		return

	# スクリプトの中身
	def script_content(self, bin_width, hist_data_path, png_path):
		#
		script = 'set term pngcairo font "Arial,14" \nset colorsequence classic \n'
		script += '# \ndata = "' + str(os.path.abspath(hist_data_path)) + '" \nset output "' + str(os.path.abspath(png_path)) + '"\n'
		#
...
class MakeBatch:
	...
	def setup_all(self):
		...
		#
		present_udf = 'Eq_' + self.target_name + "_uin.udf"
		udf_files = [template, read_udf, present_udf]
		if platform.system() == "Windows":
