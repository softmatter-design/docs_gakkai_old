...
一つは、結節点の揺らぎに注目しミクロな変形がマクロと異なるとした「ファントムネットワークモデル~\cite{James1943, Flory1976}」である。
もう一つが、「鎖の伸びきり効果」を逆ランジュバン関数で表したストランド~\cite{Kuhn1942} を用いたモデルであり、ランダムに配置された鎖を想定した「フルチェインモデル~\cite{Wu1993}」が提案されている。

ポリマーのMDシミュレーションでは、ポリマー鎖のすり抜けを抑制し絡み合いを表した 「KG 鎖」と呼ばれるビーズ・スプリングモデル~\cite{Kremer1990} が実在鎖との整合性がよいモデルとして広く用いられ、また、ビーズ間のポテンシャルを省略することで鎖のすり抜けを許容した「ファントム鎖」も表せる。
Everaers らは、規則構造を有するネットワークを用いて「KG 鎖」と「ファントム鎖」の比較を行い、各種のゴム弾性理論との高い整合性を報告している~\cite{Everaers1999}。
この規則的なネットワーク構造では、鎖のすり抜けを許容した「ファントム鎖」においてもアフィン変形する「ネオ・フッキアンモデル」としての挙動を示すことが確認された。

Flory によれば、メルト状態と同一なランダムネットワークにおいて「ファントムネットワーク」としてのふるまいを示すとされている~\cite{Flory1976}。
我々は、規則構造ネットワークのユニットセル間における規則性をランダムへと変えることで、「ファントムネットワークモデル」の構築を検討した。
...
\subsection{一軸伸長}
ファントムネットワークでは、バルク内部の架橋点の揺らぎのために伸長によるストランドのエントロピー変化が減少しバルクとしての弾性率が低下するとされている~\cite{Flory1976}。

この減少率を$F_P$と書くとこの値はシステムサイズ依存性があり、それぞれの極限で以下となる。
\vspace{-2mm}
\begin{align}
&\begin{cases}
...
\begin{minipage}{0.5\hsize}
	\begin{center}
	\includegraphics[width=60mm]{./fig/PR_BW.pdf}
...
\begin{minipage}{0.5\hsize}
	\begin{center}
	\includegraphics[width=60mm]{./fig/SS_all_BW.pdf}
